...
class Hypnogram:
    ...
    @classmethod
    def plotly_display(cls, labels, awakeings, num_class_choice, timezone, time=None, heart_rate=None, classifier_name=''):
        
        new_awakeings = awakeings.loc[awakeings['value'] == 1]
        plot_awakenings = awakeings.copy()
        ...
        if not new_awakeings.empty :
            ...
            for arr in indices:
                if len(arr) < 6:
                    prev_ind = arr[0] - 1
                    for i in arr:
                        labels.at[i, 'labels'] = labels.iloc[prev_ind]['labels']
                        plot_awakenings.at[i, 'value'] = 1
            
        x = cls.time_conversion(time, timezone)
        y = cls.int_to_label(labels['labels'].values, num_class_choice)
        ...
        labels['color'] = labels.apply(lambda row: get_color(row), axis=1)
